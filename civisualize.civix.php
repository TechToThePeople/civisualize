...
/**
 * @return CRM_Civisualize_Upgrader
 */
 ...
 */
function _civisualize_civix_civicrm_caseTypes(&$caseTypes) {
  if (!is_dir(__DIR__ . '/xml/case')) {
    return;
  }

  foreach (_civisualize_civix_glob(__DIR__ . '/xml/case/*.xml') as $file) {
 ...
 */
function _civisualize_civix_civicrm_angularModules(&$angularModules) {
  if (!is_dir(__DIR__ . '/ang')) {
    return;
  }

  $files = _civisualize_civix_glob(__DIR__ . '/ang/*.ang.php');
 ...
 */
function _civisualize_civix_civicrm_themes(&$themes) {
  $files = _civisualize_civix_glob(__DIR__ . '/*.theme.php');
  foreach ($files as $file) {
 ...
 * @return bool
 */
function _civisualize_civix_insert_navigation_menu(&$menu, $path, $item) {
  ...
  if (empty($path)) {
    ...
    foreach ($menu as $key => &$entry) {
      if ($entry['attributes']['name'] == $first) {
        ...
        $found = _civisualize_civix_insert_navigation_menu($entry['child'], implode('/', $path), $item);
      }
    }
    return $found;
  }
 ...
 */
function _civisualize_civix_navigationMenu(&$nodes) {
  if (!is_callable(['CRM_Core_BAO_Navigation', 'fixNavigationMenu'])) {
    _civisualize_civix_fixNavigationMenu($nodes);
  }
}
 ...
 * missing them.
 */
function _civisualize_civix_fixNavigationMenu(&$nodes) {
  $maxNavID = 1;
  ...
  _civisualize_civix_fixNavigationMenuItems($nodes, $maxNavID, NULL);
}

function _civisualize_civix_fixNavigationMenuItems(&$nodes, &$maxNavID, $parentID) {
  ...
  foreach ($origKeys as $origKey) {
    ...
    if (isset($nodes[$origKey]['child']) && is_array($nodes[$origKey]['child'])) {
      _civisualize_civix_fixNavigationMenuItems($nodes[$origKey]['child'], $maxNavID, $nodes[$origKey]['attributes']['navID']);
    }
  }
}
 ...
 */
function _civisualize_civix_civicrm_alterSettingsFolders(&$metaDataFolders = NULL) {
